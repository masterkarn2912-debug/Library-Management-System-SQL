...
ALTER TABLE BOOKS MODIFY Isbn Varchar (50) primary key ;

INSERT INTO books(isbn, book_name, Category, renT, status, author, publisher) 
VALUES
...
alter table BRANCH MODIFY Branch_id varchar (10) primary key;


INSERT INTO branch
(branch_id, manager_id, branch_address, contact) 
VALUES
...
alter table Employee add column position varchar (20) after Emp_name;
...
SELECT Book_name, Author, Category, Rent
FROM BOOKS
WHERE Status = 'yes';

-- Show all members who registered after 1st Jan 2022
...
-- Show total salary paid per branch
SELECT Branch_id, SUM(salary) AS total_salary
FROM EMPLOYEE
GROUP BY Branch_id;

-- Display issued books with member names
SELECT issued_status.Issued_id, members.Member_name, issued_status.Issued_book_name, issued_status.Issued_date
FROM issued_status
JOIN Members 
ON issued_status.Issued_member_id = members.Member_id;


-- Find how many books each employee has issued
SELECT Issued_emp_id, COUNT(*) AS books_issued
FROM Issued_status
GROUP BY Issued_emp_id;

-- Show branch address with manager name
SELECT Branch_id, Branch_Address, Emp_name AS Manager_Name
FROM Branch b
JOIN EMPLOYEE e
ON b.Manager_id = e.Emp_id;

-- Find members who have issued more than 1 book
...
-- Calculate total rent generated per book
SELECT book_name, COUNT(issued_id) * rent AS total_rent
FROM BOOKS 
JOIN Issued_status
ON Isbn = Issued_book_isbn
GROUP BY book_name, rent;

-- Identify members who issued books but never returned them
